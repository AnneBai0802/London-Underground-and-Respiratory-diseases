...
areas,NO2=Clean_Data_NO2.clean_NO2()
...
NO2=np.array([matrix[i][0] for i in range(5,len(matrix))])
Percentage_Death_Boroughs=np.array([matrix[i][1] for i in range(5,len(matrix))])
Name_Boroughs=np.array([matrix[i][2] for i in range(5,len(matrix))])
...
plt.legend()
plt.savefig('Percentage Death 2021 against NO2 in 2018-19',dpi=500)